...
struct hg_test_pipeline_args {
	...
	static hg_return_t
hg_test_pipeline_forward_cb(const struct hg_cb_info *callback_info)
{
	struct hg_test_pipeline_args *args =
		(struct hg_test_pipeline_args *) callback_info->arg;
		...
		unsigned int nhandles)
{
	...
	for (i = 0; i < skip; i++) {
		...
		for (j = 0; j < nhandles; j++) {
			//ret = HG_Forward(handles[j], hg_test_pipeline_forward_cb, &args, &in_struct);
			ret = HG_Forward(handles[j], hg_test_pipeline_forward_cb, &args, &in_struct);
			if (ret != HG_SUCCESS) {
