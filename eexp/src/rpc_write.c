...
//static na_addr_t svr_addr[1024];
//static num_svr = 0;

#define READLINE_LIMIT 2048 //1024
static uint32_t write_value = 0;
static uint32_t write_comp [READLINE_LIMIT];

static hg_bulk_t bhdl;
...
hg_id_t write_register(hg_class_t *hg_c, hg_context_t *context) {
	...
	hg_class = hg_c;
	hg_context = context;
	hg_id = MERCURY_REGISTER(hg_class, "hg_write", write_in_t,
...
uint32_t rpc_write(int32_t size, void *buffer, na_addr_t svr) {
	...
	ret = HG_Create(hg_context, svr, hg_id, &state->handle);
	assert(ret == HG_SUCCESS);
	(void)ret;
